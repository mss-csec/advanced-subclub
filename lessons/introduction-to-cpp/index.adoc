...
- the ``bool``, a **boo**lean type which supports the boolean values ``true`` (1) and ``false``(2).
...
In the former case, there's not enough precision and the addition results in an extra little \(4\times10^{-17}\); in the latter, ``1.0 / 0.0`` should result ``#INF``
====