...
Enter the dijoint-set data structure!

We begin with an array as long as the number of elements.
The value of each element is set to their index.
...
=== Hash Table
|===
|STL analogue|link:++http://www.cplusplus.com/reference/map/map/++[``std::map<const T,T>``]
|===
|===
|Find|Insert|Erase|Memory Usage
|stem:[\text{O}(1)]
